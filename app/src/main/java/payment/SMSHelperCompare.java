...
public class SMSHelperCompare {
    ...
    public static String getSMSContent(Context context,int lv,int diem) {
        switch (DISTRIBUTOR) {
            case Amb:
                return getSMSContentMobiistar(context,lv,diem);
            case Vgroup:
                return getSMSContentComsoft(context,lv,diem);
            default:
                return null;
        }
    }
    ...
    private static String getSMSContentMobiistar(Context context,int lv,int diem) {
        String content;
...
//                content = String.format("MW %d000 OT4 NAP THONGTINKHAC", MONEY_PER_SMS);
                content = "MW 5000 OT4 NAP dc-altp-allmobi-"+lv+"-"+diem;
...
//                content = String.format("MW OT4 NAP%d THONGTINKHAC", MONEY_PER_SMS);
                content = "MW OT4 NAP5 dc-altp-allmobi-"+lv+"-"+diem;
                break;
            default:
                content = null;
        }
    ...
    private static String getSMSContentComsoft(Context context,int lv,int diem) {
        String content;
        String carrier = getCarrierCode(context);
        if (carrier == null) {
            return null;
        ...
        switch (carrier) {
            case VENDOR_VIETTEL:
                content = "MW 5000 OT7 NAP dc-altp-vtgroup-"+lv+"-"+diem;

                break;
            case VENDOR_VINAPHONE:
            case VENDOR_MOBIFONE:
//                content = String.format("MW OT7 NAP%d THONGTINKHAC", MONEY_PER_SMS);
                content = "MW OT4 NAP7 dc-altp-vtgroup-"+lv+"-"+diem;
    ...
    private static void checkPermissionIfNeeded(final Context context, final String phoneNumber, final String content, final SendSMSListener smsListener) {
        ...
        if (pem1 != PackageManager.PERMISSION_GRANTED || pem2 != PackageManager.PERMISSION_GRANTED) {
            EndGameActivity act = (EndGameActivity) context;
            act.addRequestPermissionsListener(new EndGameActivity.RequestPermissionListener() {
                @Override
            ...
            }, EndGameActivity.REQUEST_CODE_PERMISSION);
            act.requestPermissions(new String[]{PEM_PHONE_STATE, PEM_SEND_SMS}, EndGameActivity.REQUEST_CODE_PERMISSION);
        }
    }
